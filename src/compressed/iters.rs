...
use super::node::{IntoIter as NodeIntoIter, Iter as NodeIter, MutNodeRef, NodeRef};

/// An iterator over the key-value pairs of a `CompressedBTreeMap`, in sorted order by key.
///
...
{
    pub(super) inner: NodeIter<'a, K, V, B, NodeRef<'a, K, V, B>>,
}
...
/// An iterator over the keys of a `CompressedBTreeMap`, in sorted order.
///
...
/// An iterator over the values of a `CompressedBTreeMap`, in order by key.
///
...
/// A mutable iterator over the values of a `CompressedBTreeMap`, in order by key.
///
...
where
    ...
    Sum<U1, Prod<U2, B>>: ArrayLength,
{
    pub(super) inner: NodeIter<'a, K, V, B, MutNodeRef<'a, K, V, B>>,
}
...
/// An owning iterator over the key-value pairs of a `CompressedBTreeMap`, in sorted order by key.
///
/// This struct is created by the [`into_iter`](std::iter::IntoIterator::into_iter) method on
/// [`CompressedBTreeMap`](super::wrapper::CompressedBTreeMap) (provided by the [`IntoIterator`] trait).
pub struct IntoIter<
    'a,
...
/// An owning iterator over the keys of a `CompressedBTreeMap`, in sorted order.
///
...
/// An owning iterator over the values of a `CompressedBTreeMap`, in order by key.
///
