...
use allocated::{CollectIn, CountingAllocator, TrackingAllocator};
...
#[test]
fn test_small_7_remove_first_repeatedly() -> Result<(), Box<dyn Error>> {
    let alloc = CountingAllocator::default();
    let mut btree = AllocatedBTreeMap::<u32, u32, U1>::new_in(&alloc)?;
    ...
    unsafe {
        ...
        // Remove first entry three times
        for expected_key in 1..=3 {
            let entry = btree.first_entry_in(&alloc);
            assert!(entry.is_some());
            let entry = entry.unwrap();
...
#[test]
fn test_remove_all_via_first_entry() -> Result<(), Box<dyn Error>> {
    let alloc = CountingAllocator::default();
    let mut btree = AllocatedBTreeMap::<u32, u32, U1>::new_in(&alloc)?;
    ...
    unsafe {
        ...
        // Remove all entries via first_entry
        for expected_key in 1..=5 {
            println!("{:?}", expected_key);
            let entry = btree.first_entry_in(&alloc);
            assert!(entry.is_some());
            let entry = entry.unwrap();
            assert_eq!(*entry.key(), expected_key);
...
#[test]
fn test_remove_causing_tree_height_reduction() -> Result<(), Box<dyn Error>> {
    // let alloc = CountingAllocator::default();
    let alloc = TrackingAllocator::default();
    let mut btree = AllocatedBTreeMap::<u32, u32, U1>::new_in(&alloc)?;

    unsafe {
        // Build a tree with height > 1 (13 elements with B=U1 causes split)
        for i in 1..=13 {
            btree.insert_in(&alloc, i, i * i)?;
        }
        ...
        // Remove entries until we're back to a single-level tree
        for expected_key in 1..=11 {
            let entry = btree.first_entry_in(&alloc);
            assert!(entry.is_some());
            let entry = entry.unwrap();
            assert_eq!(*entry.key(), expected_key);
...
#[test]
fn test_small_4_remove_first_twice() -> Result<(), Box<dyn Error>> {
    let alloc = CountingAllocator::default();
    let mut btree = AllocatedBTreeMap::<u32, u32, U1>::new_in(&alloc)?;
    ...
    unsafe {
        ...
        assert_eq!(btree.n, 2);
        assert_equal(btree.iter().map(|(k, v)| (*k, *v)), vec![(3, 9), (5, 25)]);
...
#[test]
fn test_small_6_remove_first_three_times() -> Result<(), Box<dyn Error>> {
    let alloc = CountingAllocator::default();
    let mut btree = AllocatedBTreeMap::<u32, u32, U1>::new_in(&alloc)?;
...
// 3. Uncomment and run: cargo test test_removal_operations_reproduce -- --nocapture
//
// #[test]
...
// Tests for underflow handling edge cases
#[test]
...
fn test_remove_all_15_elements() -> Result<(), Box<dyn Error>> {
    ...
    _assert_allocations::<_, U1>(alloc, 14);
...
fn test_remove_all_31_elements() -> Result<(), Box<dyn Error>> {
    ...
    _assert_allocations::<_, U1>(alloc, 30);
...
fn test_deep_tree_removal() -> Result<(), Box<dyn Error>> {
    ...
    _assert_allocations::<_, U1>(alloc, 62);
...
fn test_partial_removal_patterns() -> Result<(), Box<dyn Error>> {
    ...
    _assert_allocations::<_, U1>(alloc, 17);
    // assert!(alloc.outstanding().is_empty());
    // assert!(false);
...
fn test_specific_underflow_sequence() -> Result<(), Box<dyn Error>> {
    ...
    _assert_allocations::<_, U1>(alloc, 9);
...
fn test_vacant_entry_key_below_key_above() -> Result<(), Box<dyn Error>> {
    let alloc = CountingAllocator::default();
    let mut btree = wrapper::CompressedBTreeMap::<_, _, U6, _>::new_in(&alloc)?;
